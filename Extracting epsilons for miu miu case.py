...
a=0.01
l=1
eps = np.linspace( 0.1, 4 ,200 )
nuA = np.linspace( 3.95,4.2,50 )
StorEps = []
StorNu = []
count =0
for i in nuA:
...
epss = np.sqrt((48/5)*(nuA-4))#(1-np.sqrt(1-16*(nuA)*(a**2/l**2)))/2*nuA*(a/l)
epsss = (1+np.sqrt(1-16*(nuA)*(a**2/l**2)))/2*nuA*(a/l)#(1+np.sqrt(1-16*(nuA)*(a**2/l**2)))/2*nuA*(a/l)
linee = (0.01)*nuA

plt.plot(StorNu,StorEps,label = 'Numerical Solution for $\epsilon_4(\\nu)$')
#plt.plot( nuA, epsil1,label = 'Analytical Solution for $\epsilon_4(\\nu)$' )
plt.plot(nuA,epss,'r', label = 'Analytical Solution for $\epsilon_4(\\nu)$')
plt.plot(nuA,linee,'k')
plt.plot(nuA,epsss,'k')
plt.xlabel('$\\nu$')
